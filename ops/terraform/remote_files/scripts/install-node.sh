...
function install-amplify() {
  if [[ -n "${AMPLIFY_BRANCH}" ]] ; then
    install-amplify-from-source
  elif [[ -n "${AMPLIFY_VERSION}" ]] ; then
    install-amplify-from-release
  else
    echo "No amplify version or branch specified. Not installing amplify."
    exit 1
  fi
}

function install-amplify-from-release() {
  echo "Installing amplify from release ${AMPLIFY_VERSION}"
  sudo apt-get -y install --no-install-recommends jq
  wget "https://github.com/bacalhau-project/amplify/releases/download/${AMPLIFY_VERSION}/amplify_${AMPLIFY_VERSION}_linux_amd64.tar.gz"
  tar xfv "amplify_${AMPLIFY_VERSION}_linux_amd64.tar.gz"
  sudo mv ./amplify /usr/local/bin/amplify
}

function install-amplify-from-source() {
  echo "Installing Amplify from branch ${AMPLIFY_BRANCH}"
  sudo apt-get -y install --no-install-recommends jq
  git clone --depth 1 --branch ${AMPLIFY_BRANCH} https://github.com/bacalhau-project/amplify.git
  cd amplify
  GO111MODULE=on CGO_ENABLED=0 go build -gcflags '-N -l' -trimpath -o ./amplify
  sudo mv ./amplify /usr/local/bin/amplify
}
  ...
  - job_name: journal
    ...
    journal:
      ...
      labels:
        job: systemd-journal
        host: ${HOSTNAME}
        label_project: amplify
        environment: ${TERRAFORM_WORKSPACE}
    relabel_configs:
      - action: keep
        source_labels: [__journal__systemd_unit]
        regex: '^amplify\.service$'
      - source_labels: ['__journal__systemd_unit']
